...
curl -X GET "https://api.mantlz.app/api/v1/forms/form_123abc/submissions?limit=10&cursor=sub_345mno" \
  -H "X-API-Key: mk_your_api_key_here"
```
...
async function listSubmissions(formId, limit = 50, cursor = null) {
  try {
    let url = `https://api.mantlz.app/api/v1/forms/${formId}/submissions?limit=${limit}`;
