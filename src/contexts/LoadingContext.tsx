...
type LoadingContextType = {
  ...
  // Add a new function to render skeleton placeholders
  ...
  const renderSkeleton = (type: 'card' | 'list' | 'form' | 'custom', count: number = 1) => {
    switch (type) {
          ...
          <div className="space-y-2 w-full">
            {Array(count).fill(0).map((_, i) => (
              <div key={i} className="flex items-center space-x-2">
                <Skeleton className="h-4 w-4 rounded-full dark:bg-zinc-900" />
                <Skeleton className="h-4 flex-1 dark:bg-zinc-900" />
              </div>
            ))}
          </div>
        );
      case 'form':
        return (
          <div className="space-y-4 w-full">
            <Skeleton className="h-5 w-1/4 mb-1 dark:bg-zinc-900" />
            <Skeleton className="h-10 w-full mb-4 dark:bg-zinc-900" />
            <Skeleton className="h-5 w-1/4 mb-1 dark:bg-zinc-900" />
            <Skeleton className="h-10 w-full mb-4 dark:bg-zinc-900" />
            <Skeleton className="h-10 w-1/4 dark:bg-zinc-900" />
          </div>
        );
      case 'custom':
      default:
        return (
          <div className="grid grid-cols-3 gap-4 w-full ">
            <Skeleton className="h-50 w-full dark:bg-zinc-900" />
            <Skeleton className="h-50 w-full dark:bg-zinc-900" />
            <Skeleton className="h-50 w-full dark:bg-zinc-900" />
          </div>
        );
