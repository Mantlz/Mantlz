...
const EmptyState = ({
  ...
  <div className="flex flex-col items-center justify-center py-16">
    <div className="w-10 h-10 rounded-lg bg-accent flex items-center justify-center mb-5 animate-pulse">
...
export function BrowserAndLocationStats({
  ...
  const maxCountriesPages =
    Math.ceil(sortedLocations?.length / ITEMS_PER_PAGE) || 1;
                            ...
                            <Geographies geography={geoUrl}>
                              {({ geographies }) =>
                                geographies.map((geo) => {
                                  ...
                                  const geoProps = geo.properties as Record<
                                    string,
                                    string | number | boolean
                                  >;
                                  const geoName = geoProps.name as
                                    | string
                                    | undefined;
                                    ...
                                    <circle
                                      ...
                                      className={
                                        isTop3
                                          ...
                                          : "fill-orange-400 dark:fill-orange-700"
                                      }
                                      stroke="#fff"
                                      strokeWidth="1.5"
                                    />
                          ...
                          <button
                            onClick={() =>
                              setCountriesPage((prev) =>
                                Math.min(prev + 1, maxCountriesPages)
                              )
                            }
            ...
            <div className="flex justify-between items-center w-full">
              ...
              {maxBrowsersPages > 1 && (
                <div className="flex items-center space-x-1">
                  ...
                  <button
                    onClick={() =>
                      setBrowsersPage((prev) =>
                        Math.min(prev + 1, maxBrowsersPages)
                      )
                    }
                ...
                <div className="mb-5 flex flex-wrap gap-x-4 gap-y-2">
                  {browsers.slice(0, 5).map((browser, idx) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className={`w-2 h-2 rounded-lg bg-${colors[idx % colors.length]}-500`}
                        ></div>
                ...
                <div className="space-y-1.5 mt-4">
                  {paginatedBrowsers.map((browser, idx) => (
                    <StatItem
                      ...
                      color={
                        ["orange", "indigo", "violet", "orange", "cyan"][
                          idx % 5
                        ]
                      }
                      ...
                      <Geographies geography={geoUrl}>
                        {({ geographies }) =>
                          geographies.map((geo) => {
                            ...
                            const isHighlighted = locations.some((loc) => {
                              return (
                                geoName &&
                                (geoName === loc.name ||
                                  geoName.includes(loc.name) ||
