...
export function SearchResults({ 
  ...
  if (isLoading) {
    return (
      <div className="p-6 text-center">
        ...
        <p className="text-sm text-gray-600 dark:text-zinc-500">Loading results...</p>
      </div>
    )
  }
  ...
  if (!isProUser && selectedFormId === null) {
    return (
      <div className="py-12 text-center px-4">
        ...
        <p className="text-xs text-zinc-500 dark:text-zinc-500 max-w-xs mx-auto mb-4">
  ...
  if (!data || data.submissions.length === 0) {
    return (
      <div className="p-6 text-center">
        <div className="w-12 h-12 mx-auto mb-4 rounded-lg bg-white dark:bg-zinc-800 flex items-center justify-center">
          <FileSearch className="h-6 w-6 text-gray-400 dark:text-zinc-500" />
        </div>
...
function SubmissionSearchResult({ 
  ...
  return (
    <div
      onClick={onClick}
      className="flex items-center gap-3 px-3 py-3 hover:bg-zinc-50 dark:hover:bg-amber-500/40 cursor-pointer transition-colors group border-b border-zinc-200 dark:border-zinc-800 last:border-b-0"
    >
      <div className="flex-1 min-w-0">
        ...
        <div className="text-xs text-zinc-500 dark:text-zinc-500 flex items-center gap-2 flex-wrap">
          ...
          {isProUser && submission.analytics && (
            <>
              <span className="flex items-center text-xs text-gray-400 dark:text-zinc-500">
      ...
      <div className="flex items-center space-x-2 ml-auto shrink-0">
        <Button
          variant="outline"
          size="sm"
          className="h-7 w-7 p-0 text-zinc-500 hover:text-gray-700 cursor-pointer dark:text-zinc-500 dark:hover:text-gray-300 bg-white dark:bg-zinc-900 hover:bg-zinc-100 dark:hover:bg-amber-500border-zinc-200 dark:border-zinc-700"
          onClick={onViewClick}
